...
열리고, 형식을 고르면 **AI 에게 줄 지시문(대본)** 이 나온다. 코드가 아니라 대본이다.
복사해서 에이전트에 붙여 넣으면 그 형식으로 산출물을 만든다.

형식은 PDF · Excel · PowerPoint · Word · CSV 다섯이다.

**서식 규격이 대본 안에 박혀 있다.** 색 여섯 개, 글자 위계 네 단계, 표 스타일,
하지 말 것 목록이 `_style.md` 에 고정되어 모든 대본에 들어간다. 그래서 착수 때
만들든 중간에 만들든, 누가 어떤 도구로 만들든 **같은 모양이 나온다.** 이게 대본을
쓰는 이유다 — 매번 "모던하게 만들어 줘" 라고 말하면 매번 다른 것이 나온다.

**대본은 내용을 품지 않는다.** 원본 `.md` 경로만 가리키고 "이 파일을 먼저 읽어라"
라고 지시한다. 그래서 문서를 고친 뒤 같은 대본을 다시 써도 최신 내용으로 만들어지고,
HTML 크기가 문서 수와 무관하다.

대본은 `templates/script/*.md` 에 있다. `_style.md` 가 공통 서식 규격이고 빌드할 때
각 형식 대본의 `__STYLE__` 자리에 끼워진다. `__DOC_FILE__` · `__DOC_TITLE__` ·
`__DOC_STEM__` 은 모달을 열 때 브라우저에서 치환된다.

서식을 바꾸려면 **`templates/script/` 의 마크다운을 고친다.** `build.mjs` 는 파일을
읽어 나르기만 한다.
