...
제목만 확인하면 `TODO(owner)` 만 든 껍데기가 통과한다. 그러면 규칙은 있고 효과는
없는 상태가 된다 — 이 스킬이 가장 피해야 할 모양이다. `checkEli5()` 가 내용을 본다.

| 무엇 | 결과 |
| --- | --- |
| `TODO(...)` 가 남아 있다 | **실패** — 빌드가 막힌다 |
| 한 줄 요약(`> `)이 비어 있다 | **실패** |
| 한 문장이 60자를 넘는다 | 경고 |
| 약어를 풀지 않았다 | 경고 (`AI`·`API`·`PDF` 등 통용 약어는 면제) |
| `왜 필요한가` · `읽어야 하는 사람` 이 없다 | 경고 |

**못 채운 것은 실패, 문체는 경고**로 나눈 이유가 있다. 문체를 실패로 걸면 기존
문서가 한꺼번에 막히고, 그러면 아무도 고치지 않는다.

### 빌더가 그리는 화면도 ELI5 를 따른다

홈 · 추적성 매트릭스 · 검색 · 연동 서비스 · 디자인 시안 · 가이드는 `.md` 가 아니라
`build.mjs` 가 직접 그리므로 스키마 검증을 거치지 않는다. 그래서 **각 화면 안내문을
같은 규칙으로 직접 쓴다** — 왜 필요한지 먼저, 비유 하나, 짧게.

화면을 새로 추가하면 `home-lead` 문단을 함께 쓴다. 제목만 두면 그 화면은
"뭔지 모르겠는 탭"이 된다.
...
`DOC_SHAPE` 는 **시트 이름과 열까지 적는다.** 원칙만 주면 흉내만 낸 파일이 나온다.
...
문서 `name` 으로 찾고, 없으면 `DOC_SHAPE_ALIAS` 를 따라간다. 문서 이름이 바뀌어도
(`prd` → `requirements`) 이미 만들어진 프로젝트가 폼을 잃지 않는다.
둘 다 없으면 `DOC_SHAPE_DEFAULT` — 결론을 앞으로, 행 수가 가장 많은 표를 본표로.

**새 문서 종류를 넣으면 `DOC_SHAPE` 에 항목을 추가한다.** 안 넣어도 동작하지만
그 문서는 "원본 순서대로"가 된다.

### `_form.md` — 폼 문법은 실제 산출물에서 뽑았다

원칙을 아무리 적어도 "정돈된 표"가 무엇인지는 전해지지 않는다. 실제로 고객에게
나간 산출물(`구축범위&마일스톤.xlsx`)을 열어 구조를 뽑아 규칙으로 옮겼다.
...
| **계층은 병합** | 같은 상위 값을 반복하지 않는다. **가장 작은 단위가 한 행** |
| 통번호 | 분류마다 1부터 다시 시작하지 않는다 |
| **주 단위 간트** | 월 4칸 병합 + `1w 2w 3w 4w` 두 줄 머리행, 막대는 **셀 배경색**(`#B7B7B7`), 칸에 글자 없음 |
| 오픈 차수 | 간트 머리행 위에 `1차` `2차` 를 해당 주 위치에 |

**계층을 병합으로 묶고 최소 단위를 행으로 쪼개는 것**이 핵심이다. 그래야 고객이
"이 화면에 기능이 몇 개인지"를 눈으로 센다. 마크다운 표를 그대로 옮기면 이 모양이
되지 않는다.

`_form.md` 는 `pdf` · `excel` · `pptx` · `docx` 대본에 들어간다. CSV 는 병합·간트가
성립하지 않아 넣지 않는다.

### `_chart.md` — 눈으로 보이는 시트를 하나는 넣는다

표만 있는 산출물은 **읽어야** 알 수 있다. 고객이 **보고** 알 수 있는 시트를 하나
이상 넣게 한다. `excel` · `pptx` 대본에 들어간다.
...
**막대는 셀 배경색으로 칠한다.** 엑셀 기본 차트를 쓰지 않는다 — 인쇄·PDF 변환에서
깨지지 않고, 차트 XML 손상으로 파일이 복구되는 일도 없다.

간트 규격은 실제로 생성해 보고 쓸 만하다고 확인된 형태를 그대로 옮겼다.
...
진행 `#1C1B19`, 위험·미해소 `#C0392B`, 배경 트랙 `#FAFAF8`.
열 폭은 A `3` · ID `12` · 항목 `30` · 시작·종료·기간 각 `10` · 주차 `3.2`.
칸에 글자를 넣지 않고 색만으로 표현하며, 4주마다 세로 구분선을 넣는다.

**착수일이 없으면 날짜를 지어내지 않는다.** `W1`·`W2` 상대 주차로 적고 주석에
이유를 남긴다.

`범례(보는 법)`와 `주석`은 필수다. 색만 칠해 두면 받는 사람이 무슨 색인지 묻는다.

### 내용이 없어도 시트를 만든다 — `(예시)` 로

일정·건수가 전혀 없는 문서도 시트를 만든다. 다만 **속이지 않는다.**

1. 시트 이름과 제목에 `(예시)` 를 붙인다
2. 제목 아래 굵게 — **막대는 예시입니다. 합의된 일정이 아닙니다.**
3. 행은 **원본에 있는 항목 이름으로** 채운다. 항목까지 지어내지 않는다
4. 기간은 균등 배분하고 주석에 "협의 전 균등 배분" 이라고 적는다
5. 미정 항목은 위험색으로 칠해 눈에 걸리게 둔다

빈 시트는 아무 말도 하지 않는다. 폼이 보이면 고객이 "여기에 우리 일정을 넣으면
되겠다" 를 안다.

문서별 기본 차트는 `DOC_SHAPE` 에 `**차트 시트**:` 한 줄로 적는다. 적지 않은
문서는 위 표의 `그 밖` 규칙을 따른다.
