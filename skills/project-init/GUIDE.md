...
> 코드를 만들기 **전에** 무엇을 만들지 정한 내용이 이 사이트에 모여 있습니다.

**왜 필요한가** — 정하지 않고 만들면 각자 다른 것을 떠올립니다.
그 차이는 만든 뒤에 드러나고, 그때 고치는 비용이 가장 큽니다.

**비유하자면** — 집을 짓기 전 도면을 모아 둔 서랍입니다.
도면이 없으면 벽을 세운 뒤에 문 위치를 다시 정하게 됩니다.

**이 페이지를 읽어야 하는 사람** — 이 문서 모음을 처음 여는 모든 사람.
