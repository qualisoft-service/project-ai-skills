 ...
 *
 * **시트 이름과 열까지 적는다.** 원칙만 주면 흉내만 낸 파일이 나온다.
 * 키는 문서 name(모듈에 적힌 것)이다. 없으면 기본 지침을 쓴다. */
const DOC_SHAPE = {
  "wbs-schedule": [
    "이 문서는 **일정표**입니다. 시트를 두 개로 나눕니다.",
    "",
    "**`1. 마일스톤`** — 고객이 가장 먼저 보는 시트입니다.",
    "상단 요약(`총 기간`, `전략` 차수별)을 두고, 그 아래 **주 단위 간트**를 그립니다.",
    "열은 `Level | Task | Memo | 1 month(1w 2w 3w 4w) | 2 month | …` 이고,",
    "`Level` 은 공정명으로 병합합니다. 막대는 셀 배경색으로 칠하고 글자를 넣지 않습니다.",
    "간트 머리행 위에 `1차` `2차` 오픈 차수를 해당 주 위치에 세웁니다.",
    "",
    "**`2. 과업목록`** — 열은 `공정 | 번호 | 과업 | 담당 | 시작 | 종료 | 진척 | 산출물 | 비고`.",
    "`공정` 은 병합하고 번호는 전체 통번호로 붙입니다. **지연 과업은 굵게** 표시합니다.",
    "",
    "핵심 수치는 `전체 과업`, `종합 진척률`, `진행 중`, `지연` 입니다.",
      "",
    "**차트 시트**: `3. 간트` — 과업을 주차 막대로. 지연 과업은 위험색으로 칠합니다.",
  ].join("\n"),

  roadmap: [
    "이 문서는 **구축범위와 일정**입니다. 고객이 계약 범위를 확인하는 문서이므로",
    "시트를 두 개로 나눕니다.",
    "",
    "**`1. 구축범위`** — 열은 `항목 | 번호 | 주요화면 | 구성 | 설명 | 기능 | 비고`.",
    "**기능 하나가 한 행**이고, `항목`(공통·포털별 대분류)·`주요화면`·`구성`·`설명` 은",
    "병합으로 묶습니다. 번호는 전체 통번호입니다. 이러면 고객이 화면별 기능 수를",
    "눈으로 셉니다. 원본에 기능이 문단으로 적혀 있으면 **한 기능씩 행으로 쪼개세요.**",
    "",
    "**`2. 마일스톤`** — 상단에 `총 기간`과 차수별 `전략`, 그 아래 **주 단위 간트**.",
    "열은 `Level | Task | Memo | 1 month(1w 2w 3w 4w) | …` 이고 `Level` 은 병합,",
    "막대는 셀 배경색입니다. 간트 위에 오픈 차수를 세웁니다.",
    "",
    "핵심 수치는 `전체 기능 수`, `총 기간`, `오픈 차수`, `마일스톤 수` 입니다.",
      "",
        "**차트 시트**: `3. 간트` — 마일스톤을 주차 막대로. 미정 항목은 위험색으로 칠합니다.",
  ].join("\n"),

  "deliverable-register": [
    "이 문서는 **납품 목록**입니다.",
    "",
    "**`1. 산출물목록`** — 열은 `공정 | 번호 | 코드 | 산출물명 | 필수 | 고객공유 |",
    "담당 | 상태 | 비고`. `공정` 은 병합하고 번호는 전체 통번호입니다.",
    "`필수`·`고객공유`·`상태` 는 글자 그대로 씁니다(아이콘·색 알약 금지).",
    "",
    "**`2. 제외항목`** — 열은 `산출물명 | 제외 사유 | 합의 일자`.",
    "**이 시트를 빠뜨리지 마세요.** 고객이 '이건 왜 없냐'고 묻는 지점입니다.",
    "",
    "버전 이력은 마지막 시트에 둡니다. 핵심 수치는 `전체 종수`, `필수`,",
    "`작성 완료`, `미착수` 입니다.",
      "",
    "**차트 시트**: `3. 진행현황` — 공정별 진행바 + 필수/선택 건수 막대.",
  ].join("\n"),

  requirements: [
    "이 문서는 **요구사항 추적표**입니다.",
    "",
    "**`1. 요구사항`** — 열은 `부서 | 번호 | 영역 | 현행 문제 | 요구 내용 |",
    "타당성 | 우선순위 | 연계 과업 | 연계 산출물 | 비고`. `부서` 와 `영역` 은",
    "병합합니다. **연계 과업과 산출물이 같은 행에 보여야** 추적표 구실을 합니다.",
    "",
    "**`2. 미수용`** — 열은 `번호 | 요구 내용 | 미수용 사유 | 대안 | 통보 여부`.",
    "거절한 요구를 감추지 마세요. 기록이 없으면 같은 요구가 다시 올라옵니다.",
    "",
    "핵심 수치는 `전체 요구사항`, `수용`, `추가 검토`, `미수용` 입니다.",
      "",
        "**차트 시트**: `3. 분포` — 부서별 건수 막대 + 타당성별 막대.",
  ].join("\n"),

  "screen-spec": [
    "이 문서는 **화면 설계서**입니다.",
    "",
    "**`1. 화면목록`** — 열은 `구분 | 번호 | 화면군 | 화면명 | 설명 | 주요 기능 | 비고`.",
    "**기능 하나가 한 행**이고 `구분`·`화면군`·`화면명`·`설명` 은 병합합니다.",
    "`구축범위` 와 같은 모양입니다.",
    "",
    "**`2. 화면상세`** 부터는 화면마다 한 시트 또는 한 쪽. 화면마다 **무엇을 하는",
    "화면인지 한 줄**을 맨 앞에 붙이고, 그 아래 요소·상태·검증 규칙을 표로 둡니다.",
    "",
    "핵심 수치는 `전체 화면 수`와 구분별 개수입니다.",
      "",
    "**차트 시트**: `3. 화면수` — 구분별 화면 수 가로 막대.",
  ].join("\n"),

  "project-brief": [
    "이 문서는 **사업 개요**입니다.",
    "",
    "**`1. 개요`** — 목표(`번호 | 목표 | 성공 판단 기준`)와 범위를 둡니다.",
    "**`범위`는 포함과 제외를 한 표에 `구분 | 내용` 으로 나란히** 둡니다.",
    "제외를 흐리게 적으면 나중에 분쟁이 됩니다.",
    "",
    "**`2. 이해관계자`** — 열은 `역할 | 소속 | 담당 | 결정 권한`.",
    "**연락처·이메일은 넣지 않습니다.** 이름과 역할만 남깁니다.",
    "",
    "핵심 수치는 `목표 수`, `총 기간`, `이해관계자 수` 입니다.",
  ].join("\n"),

  "as-is-to-be": [
    "이 문서는 **업무 개선안**입니다. 현행과 개선을 **같은 표에서 나란히** 보여줍니다.",
    "열은 `업무 | 번호 | 현행(As-Is) | 문제점 | 개선(To-Be) | 기대 효과 | 비고`.",
    "`업무` 는 병합합니다. 현행과 개선을 다른 시트로 떼면 비교가 안 됩니다.",
    "",
    "핵심 수치는 `개선 대상 업무 수`, `자동화 항목`, `유지 항목` 입니다.",
      "",
    "**차트 시트**: `2. 개선효과` — 업무별 현행 대비 개선 흐름 띠.",
  ].join("\n"),

  "data-model": [
    "이 문서는 **데이터 구조**입니다.",
    "",
    "**`1. 테이블목록`** — 열은 `구분 | 번호 | 논리명 | 물리명 | 설명 | 주요 관계`.",
    "고객이 읽으므로 **한글 논리명을 물리명보다 앞에** 둡니다.",
    "",
    "**`2. 칼럼정의`** — 열은 `테이블 | 번호 | 논리명 | 물리명 | 타입 | 필수 |",
    "기본값 | 설명`. `테이블` 은 병합합니다.",
    "",
    "핵심 수치는 `테이블 수`, `칼럼 수`, `관계 수` 입니다.",
      "",
    "**차트 시트**: `3. 관계도` — 테이블 × 테이블 참조 매트릭스.",
  ].join("\n"),

  "api-spec": [
    "이 문서는 **연동 규약**입니다.",
    "열은 `구분 | 번호 | 기능 | 메서드 | 경로 | 요청 | 응답 | 인증 | 비고`.",
    "`구분` 은 병합합니다. 외부 시스템 연동은 별도 시트로 `대상 시스템 |",
    "연동 방식 | 주기 | 담당 | 상태` 를 둡니다 — 고객사 담당자가 확인할 항목입니다.",
    "",
    "핵심 수치는 `엔드포인트 수`, `외부 연동 수` 입니다.",
      "",
    "**차트 시트**: `3. 연동현황` — 대상 시스템별 건수 막대 + 상태 진행바.",
  ].join("\n"),

  "qa-test-plan": [
    "이 문서는 **검증 계획**입니다. 고객이 무엇을 언제 확인해야 하는지가 핵심입니다.",
    "",
    "**`1. 검증범위`** — 열은 `구분 | 번호 | 시나리오 | 사전 조건 | 검증 절차 |",
    "통과 기준 | 담당 | 일정`. **`담당` 열에 `고객`·`수행사`를 분명히 적어** 고객이",
    "직접 하는 확인(UAT)과 우리가 하는 확인을 구분하세요.",
    "",
    "**`2. UAT 일정`** — 고객이 확인할 항목만 추려 `일자 | 대상 | 참여자 | 준비물`.",
    "",
    "핵심 수치는 `시나리오 수`, `고객 확인 항목`, `UAT 기간` 입니다.",
      "",
    "**차트 시트**: `3. 진행현황` — 단계별 진행바 + 고객·수행사 비중 막대.",
  ].join("\n"),

  "release-ops": [
    "이 문서는 **오픈·운영 안내**입니다.",
    "",
    "**`1. 오픈절차`** — 열은 `단계 | 번호 | 작업 | 담당 | 예정 일시 | 선행 조건 |",
    "확인 방법`. `단계` 는 병합합니다.",
    "**`2. 고객 준비사항`** — 고객이 해야 할 일만 추려 `항목 | 기한 | 담당 | 비고`.",
    "**`3. 교육·지원`** — 대상·일정·지원 기간·연락 창구.",
    "",
    "핵심 수치는 `오픈 예정일`, `교육 대상 인원`, `지원 기간` 입니다.",
      "",
    "**차트 시트**: `4. 오픈일정` — 단계별 주차 막대.",
  ].join("\n"),

  "design-system": [
    "이 문서는 **디자인 규칙**입니다. 색·글자·간격 토큰을 **견본과 함께** 보여줍니다.",
    "색 표에는 `이름 | 값(HEX) | 견본 | 쓰임` 열을 두고 `견본` 칸은 그 색으로 칠합니다.",
    "글자 표는 `이름 | 크기 | 굵기 | 줄간격 | 쓰임` 이고 실제 크기로 보여주면 좋습니다.",
    "",
    "핵심 수치는 `색 토큰 수`, `글자 단계`, `컴포넌트 수` 입니다.",
  ].join("\n"),
};

const DOC_SHAPE_DEFAULT = [
  "이 문서 종류에는 정해진 폼이 없습니다. 아래를 따르세요.",
  "",
  "1. **결론과 전체 그림을 앞으로 올리세요.** 원본 절 순서를 그대로 따르지 않습니다.",
  "2. 표가 여러 개면 **행 수가 가장 많은 표**를 본표로 세우고 나머지를 뒤로 보냅니다.",
  "3. 본표는 `구분 | 번호 | 항목 | 설명 | 비고` 골격으로 만들고, **가장 작은 단위가",
  "   한 행**이 되게 쪼갠 뒤 상위 분류는 병합으로 묶습니다.",
  "4. 번호는 전체 통번호로 붙입니다.",
  "5. 원본이 문단 위주라면 표로 억지로 바꾸지 말고, 절마다 소제목 + 본문으로 두되",
  "   맨 앞 목적 블록은 반드시 넣습니다.",
].join("\n");

/* 구버전·유사 문서 이름을 같은 폼으로 잇는다.
 * 문서 이름이 바뀌어도 이미 만들어진 프로젝트가 폼을 잃지 않게 한다. */
const DOC_SHAPE_ALIAS = {
  prd: "requirements",
  "as-is-analysis": "as-is-to-be",
  "to-be-process": "as-is-to-be",
  "integration-spec": "api-spec",
  "training-adoption": "release-ops",
  "content-model": "information-architecture",
  "performance-budget": "tech-architecture",
  "security-audit": "tech-architecture",
  "accessibility-spec": "design-system",
  "interaction-motion": "design-system",
  "user-flow": "screen-spec",
  "value-proposition": "audience-persona",
  "analytics-measurement": "qa-test-plan",
};

Object.assign(DOC_SHAPE, {
  "information-architecture": [
    "이 문서는 **메뉴 구조(IA)** 입니다. 고객이 메뉴 이름을 확인하는 문서입니다.",
    "열은 `구분 | 번호 | 1단 메뉴 | 2단 메뉴 | 3단 메뉴 | 화면 | 권한 | 비고`.",
    "**상위 메뉴는 병합**하고 가장 깊은 항목이 한 행이 됩니다. 번호는 전체 통번호.",
    "메뉴 이름은 **고객이 화면에서 보게 될 그 문구 그대로** 적습니다.",
    "",
    "핵심 수치는 `1단 메뉴 수`, `전체 화면 수`, `권한 구분 수` 입니다.",
      "",
    "**차트 시트**: `3. 메뉴수` — 1단 메뉴별 하위 화면 수 가로 막대.",
  ].join("\n"),

  "role-permission": [
    "이 문서는 **권한 매트릭스**입니다. 표 모양이 정해져 있습니다.",
    "",
    "**행은 기능·메뉴, 열은 역할**입니다.",
    "`구분 | 번호 | 메뉴·기능` 다음에 역할마다 한 열씩 놓고, 칸에는",
    "`조회`·`등록`·`수정`·`삭제`·`-` 만 씁니다. 여러 권한이면 `조회·수정` 처럼 적습니다.",
    "역할이 다섯을 넘으면 권한 등급으로 묶어 열을 줄이세요.",
    "",
    "**역할 정의를 별도 시트에** 둡니다 — `역할 | 설명 | 해당 인원 | 승인자`.",
    "핵심 수치는 `역할 수`, `통제 대상 기능 수` 입니다.",
      "",
    "**차트 시트**: `3. 권한맵` — 역할 × 기능 매트릭스를 색 농도로 칠합니다.",
  ].join("\n"),

  "audience-persona": [
    "이 문서는 **사용자 정의**입니다. 문단으로 흐르면 읽히지 않습니다.",
    "열은 `구분 | 사용자 | 역할·상황 | 목표 | 불편한 점 | 이 서비스에서 얻는 것`.",
    "사용자 유형 하나가 한 행입니다. **의사결정자와 실사용자가 다르면 행을 나눠**",
    "분명히 구분하세요 — 고객사에서 결재하는 사람과 쓰는 사람이 다릅니다.",
    "",
    "핵심 수치는 `사용자 유형 수`, `예상 사용 인원` 입니다.",
  ].join("\n"),

  "risk-open-questions": [
    "이 문서는 **리스크 대장**입니다. 고객에게 보낼 때는 **결정을 요청하는 문서**입니다.",
    "열은 `등급 | 번호 | 항목 | 현재 상황 | 영향 | 필요한 결정 | 결정 주체 | 기한`.",
    "`등급` 으로 묶어 병합하고 **Blocker 를 맨 위**에 둡니다.",
    "`결정 주체` 가 고객인 항목만 추려 별도 시트 `고객 결정 요청` 을 만드세요.",
    "",
    "핵심 수치는 `전체 항목`, `Blocker`, `고객 결정 대기` 입니다.",
      "",
    "**차트 시트**: `3. 등급분포` — 등급별 건수 막대. Blocker 는 위험색.",
  ].join("\n"),

  "decision-log": [
    "이 문서는 **의사결정 기록**입니다. 시간순으로 쌓인 기록이므로 표 하나로 냅니다.",
    "열은 `일자 | 번호 | 결정 사항 | 배경 | 근거 | 영향 | 결정 주체`.",
    "**최신이 위로** 오게 정렬합니다. 고객이 '언제 무엇을 합의했는지' 찾는 문서입니다.",
    "",
    "핵심 수치는 `전체 결정 수`, `기간`, `고객 합의 항목 수` 입니다.",
  ].join("\n"),

  glossary: [
    "이 문서는 **용어표**입니다. 열은 `분류 | 용어 | 뜻 | 비고`.",
    "`분류` 는 병합하고 용어는 **가나다순**으로 정렬합니다.",
    "약어는 `용어` 칸에 `UAT (User Acceptance Test)` 처럼 풀어 함께 적습니다.",
    "",
    "핵심 수치는 `분류 수`, `용어 수` 입니다.",
  ].join("\n"),

  "data-migration": [
    "이 문서는 **데이터 이행 계획**입니다.",
    "열은 `구분 | 번호 | 대상 데이터 | 원천 | 대상 | 건수 | 변환 규칙 |",
    "검증 방법 | 담당 | 일정`. `구분` 은 병합합니다.",
    "**`건수` 와 `검증 방법` 을 비우지 마세요** — 고객이 가장 먼저 확인합니다.",
    "이행 리허설 일정을 별도 시트에 둡니다.",
    "",
    "핵심 수치는 `대상 테이블 수`, `총 건수`, `이행 예정일` 입니다.",
      "",
    "**차트 시트**: `3. 이행일정` — 대상별 주차 막대 + 건수 막대.",
  ].join("\n"),

  "seo-spec": [
    "이 문서는 **검색 노출 설계**입니다.",
    "열은 `구분 | 번호 | 페이지 | 제목(title) | 설명(description) | 주요 키워드 | 비고`.",
    "`구분` 은 병합합니다. 제목·설명은 **실제 들어갈 문구 그대로** 적습니다.",
    "",
    "핵심 수치는 `대상 페이지 수`, `주요 키워드 수` 입니다.",
  ].join("\n"),

  "copy-guide": [
    "이 문서는 **문안 규칙**입니다. 규칙만 나열하면 지켜지지 않습니다.",
    "열은 `구분 | 규칙 | 좋은 예 | 나쁜 예` 로 두어 **예시를 나란히** 보여주세요.",
    "금지 표현은 별도 시트에 `표현 | 대신 쓸 말 | 이유` 로 둡니다.",
    "",
    "핵심 수치는 `규칙 수`, `금지 표현 수` 입니다.",
  ].join("\n"),

  "brand-messaging": [
    "이 문서는 **메시지 정의**입니다. 핵심 문구가 먼저 보여야 합니다.",
    "맨 앞에 한 줄 슬로건과 세 문장 요약을 크게 두고, 그 아래 표로",
    "`대상 | 전달할 메시지 | 근거 | 쓰이는 곳` 을 둡니다.",
    "",
    "핵심 수치는 `메시지 수`, `대상 구분 수` 입니다.",
  ].join("\n"),

  "tech-architecture": [
    "이 문서는 **기술 구성**입니다. 고객에게는 **선택 근거와 비용**이 핵심입니다.",
    "열은 `구분 | 항목 | 선택 | 버전 | 선택 근거 | 대안 | 비고`. `구분` 은 병합.",
    "성능·보안 기준은 별도 시트에 `항목 | 목표 | 측정 방법 | 현재` 로 둡니다.",
    "**내부 서버 주소·계정·키는 절대 넣지 않습니다.**",
    "",
    "핵심 수치는 `주요 구성 요소 수`, `외부 의존 수` 입니다.",
  ].join("\n"),
});
...
function loadScripts(project) {
  ...
  const form = part("_form");
  const chart = part("_chart");
  const out = {};
...
function docShape(id = "") {
  ...
  const key = DOC_SHAPE[name] ? name : DOC_SHAPE_ALIAS[name] || name;
  return "## 이 문서의 구성\n\n" + (DOC_SHAPE[key] || DOC_SHAPE_DEFAULT);
}
